...
4. **Atualização do RUNTIME_SUMMARY.cs (Doc-as-Code):** Se você alterar qualquer assinatura pública, método ou classe no Runtime, deve atualizar imediatamente o arquivo correspondente em `src/Runtime/Docs/RUNTIME_SUMMARY.cs` adicionando stubs limpos com comentários XML (`///`) e exemplos práticos.
5. **Atualização de Arquitetura:** Se a alteração mudar o fluxo de dados do ecossistema (ex: o Inspector lendo memória viva, ou adição de um subsistema de física ao EntityManager), atualize imediatamente o arquivo `docs/AI_ARCHITECTURE.md` para refletir o novo paradigma do sistema.
6. **Critério de Conclusão (Definition of Done):** Nenhuma tarefa está finalizada, nenhum Git Push deve ser feito e o /goal NÃO DEVE ser encerrado até que o log de alterações tenha sido refletido na documentação modular e stubs de código. Se você escrever código que o usuário (ou outro agente) não saberá como invocar amanhã lendo a documentação, você falhou na tarefa.